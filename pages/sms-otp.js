...
export default function SmsOTP() {
  ...
  const handleCancelOrder = async (orderID, serviceName, bypass=false) => {
    setLoading(true)
                    ...
                    {(size(listActivations?.dataLists) > 0) && <><b>Active Orders</b><br/><br/></>}
                      {
                        listActivations?.dataLists?.map(item => (<>
                          <Accordion expanded={expanded === item?.order_id} onChange={handleChangeAccordion(item?.order_id)}>
                            <AccordionSummary
                              expandIcon={
                                (item?.status === 'Waiting for SMS' || item?.status === 'Waiting for Retry SMS' || item?.status === 'Waiting for Resend SMS') ? 
                            ...
                            >
                              <Typography sx={{ width: 'auto', flexShrink: 0 }}>
                                ...
                                <img
                                  ...
                                  srcSet={`https://flagcdn.com/w40/${item?.country_code?.toLowerCase()}.png 2x`}
                                  alt=""
                                  style={{verticalAlign: 'middle'}}
                                />&nbsp;<b onClick={() => {navigator.clipboard.writeText(item?.number); alert('Text Copied.');}}
                                >{parsePhoneNumber(`+${item?.number}`).formatInternational()}</b>
                                  &nbsp;-&nbsp;<Countdown date={(item?.created_date) ? moment(item?.created_date).valueOf() + 1190000 : new Date()}
                                    renderer={({ hours, minutes=0, seconds=0, completed=false }) => {
                                      if (completed && !onCanceling){
                                        // setOnCanceling(true)
                                        // handleCancelOrder(item?.order_id, item?.service_name, true)
                                        return false
                                      } else {
                                        return <span>{minutes ?? 0}:{seconds ?? 0}</span>
                                      }
                                    }}
                                  />
                                </p>
                                ...
                                <p><b style={{ color: '#0EAC40', position: 'absolute', right: '50px', top: '25px' }}>
                                  {item?.status} 
                            ...
                            <AccordionDetails>
                              <Typography onClick={() => {navigator.clipboard.writeText(item?.sms_text); alert('Text Copied.');}}>
                            ...
                            <Accordion expanded={expanded === item?.order_id} onChange={handleChangeAccordion(item?.order_id)}>
                              ...
                              >
                                <Typography sx={{ width: 'auto', flexShrink: 0 }}>
                                  ...
                                  style={{verticalAlign: 'middle'}}
                                  alt=""
                                /> <b onClick={() => {navigator.clipboard.writeText(item?.number); alert('Text Copied.');}}>{parsePhoneNumber(`+${item?.number}`).formatInternational()}</b></p>
                                  <p><i>{item?.service_name}</i>&nbsp;-&nbsp;{currency_format(item?.price_user / parseFloat(Store.get('curs')?.curs_usd))}
                                  <b style={{ color: '#0EAC40', position: 'absolute', right: '50px', top: '25px' }}>
